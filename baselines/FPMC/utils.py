...
class Corpus(object):
    def __init__(self, ItemId, TestItemId):
        self.dict = Dictionary()
        for item in ItemId:
            self.dict.add_item(item, "train")
        for item in TestItemId:
            self.dict.add_item(item, "test")
        self.dict.prep_dict()
...
def data_generator(args):
    ...
    if not os.path.exists(path_to_data + args.data + '_train_tr.json'):
        ...
        ###
        corpus_item = Corpus(tr_df['ItemId'], test_df['ItemId'])
        ###
        # corpus_user = Corpus(tr_df['UserId'])
        ###
        corpus_user = Corpus(tr_df['UserId'], test_df['UserId'])
        ###
    ...
    # 这里需要改成训练集的session个数,直接看xxx_test文件的sid第一个编�?    # yc4_3
    # n_users = 1071676
...
def prepare_eval_test(data, batch_size, max_test_len=100):
    ...
    if num_batches * batch_size < len(all_u):
        batches.append(
            (all_u[num_batches * batch_size:], all_inp[num_batches * batch_size:], all_pos[num_batches * batch_size:]))
...
def preprocess_session(dname):
    ...
    train.to_csv(data_path + dname + '/' + dname + '_train_tr.txt', sep='\t', columns=columns, header=header,
                 index=False)
    test_.to_csv(data_path + dname + '/' + dname + '_test.txt', sep='\t', columns=columns, header=header, index=False)
    val_.to_csv(data_path + dname + '/' + dname + '_train_valid.txt', sep='\t', columns=columns, header=header,
                index=False)
