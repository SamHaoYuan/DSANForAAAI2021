...
def evaluate_gpu(gru, test_data, items=None, session_key='SessionId', item_key='ItemId', time_key='Time', cut_off=20, batch_size=100, mode='conservative'):
    ...
    # todo
    NDCG = ((ranks <= cut_off) / np.log2(ranks + 1)).sum()
    # todo
    evaluate = theano.function(inputs=[X, Y, M] + C, outputs=[REC, MRR, NDCG], updates=updatesH, allow_input_downcast=True, on_unused_input='ignore')
    ...
    while not finished:
        ...
        for i in range(minlen-1):
            ...
            if items is not None:
                ...
                y = out_idx
            rec, m, ndc = evaluate(in_idx, y, len(iters), *cidxs) # todo
            recall += rec
            mrr += m
            ndcg += ndc  # todo
            n += len(iters)
    ...
    return recall/n, mrr/n, ndcg/n  # todo
